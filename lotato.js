...
// ==========================================
// 1. FONCTION API CALL CORRIGÉE POUR L'ENVOI DU TOKEN
// ==========================================
async function apiCall(url, method = 'GET', body = null) {
    ...
    // CORRECTION CRITIQUE: Envoyer le token dans l'en-tête Authorization avec Bearer
    if (authToken) {
...
async function checkAuth() {
    ...
    authToken = token;
    console.log('Token défini:', authToken);
    ...
    try {
        console.log('Appel API authCheck...');
...
async function loadDataFromAPI() {
    try {
        ...
        if (!currentUser) {
            console.log('Utilisateur non connecté, vérification auth...');
            if (!await checkAuth()) {
                return;
            }
        }
        ...
        console.log('Données tickets:', ticketsData);
        ...
        console.log('Chargement infos entreprise...');
...
// ==========================================
// 3. FONCTION CORRIGÉE: saveTicketAPI()
// ==========================================
async function saveTicketAPI(ticketData) {
    console.log('💾 Sauvegarde ticket API appelée:', ticketData);
    ...
    try {
        ...
        const ticketToSend = {
            ...
            draw_time: ticketData.drawTime, // Note: underscore pour l'API
        ...
        console.log('📤 Données envoyées au serveur:', ticketToSend);
...
async function saveTicket() {
    ...
    console.log('🎫 Ticket préparé:', ticket);
    ...
    try {
        ...
        if (response && response.success) {
            ...
            console.log(`✅ Ticket sauvegardé #${response.ticket.number}, prochain: ${ticketNumber}`);
            ...
            const errorMsg = response ? (response.error || response.message) : 'Erreur inconnue';
...
async function saveMultiDrawTicketAPI(ticket) {
    try {
        console.log('Sauvegarde fiche multi-tirages API:', ticket);
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('save-print-ticket').addEventListener('click', async function() {
        ...
        await saveAndPrintTicket();
    });
    
    document.getElementById('save-ticket-only').addEventListener('click', async function() {
...
// ==========================================
// FONCTIONS EXISTANTES (avec corrections mineures)
// ==========================================
