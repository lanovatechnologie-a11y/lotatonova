...
// ==========================================
// 1. FONCTION DE COMMUNICATION API (AMÉLIORÉE)
// ==========================================
async function apiCall(url, method = 'GET', body = null) {
    ...
    // IMPORTANT: Utiliser l'Authorization Bearer comme dans server.js
    if (authToken) {
        headers['Authorization'] = `Bearer ${authToken}`;
    }

    // Garder aussi x-auth-token pour compatibilité
    if (authToken) {
...
// ==========================================
// 2. FONCTION DE VÉRIFICATION D'AUTHENTIFICATION (AMÉLIORÉE)
// ==========================================
async function checkAuth() {
    ...
    // Mettre à jour le token dans localStorage s'il vient de l'URL
    if (tokenFromUrl) {
    ...
    try {
        ...
        if (response && response.success) {
            ...
            console.log('Utilisateur connecté avec succès:', currentUser);
            
            // Stocker les informations utilisateur dans localStorage
            localStorage.setItem('nova_user_data', JSON.stringify(currentUser));
            localStorage.setItem('nova_user_role', currentUser.role);
        ...
        // Si c'est une erreur réseau, on ne déconnecte pas immédiatement
        if (error.message.includes('Session expirée')) {
            handleLogout();
            return false;
        }
        
        // Pour les autres erreurs, on peut essayer de continuer
        console.log('Erreur réseau, tentative de récupération des données utilisateur depuis localStorage');
        const savedUserData = localStorage.getItem('nova_user_data');
        if (savedUserData) {
...
// ==========================================
// 4. FONCTION DE SAUVEGARDE DE TICKET (CORRIGÉE)
// ==========================================
async function saveTicketAPI(ticketData) {
    try {
        ...
        const ticketToSend = {
            ...
            draw_time: ticketData.drawTime, // Note: server.js attend "draw_time" (avec underscore)
            bets: ticketData.bets,
            total: ticketData.total,
            agent_id: currentUser.id, // Utiliser l'ID de l'utilisateur connecté
            agent_name: currentUser.name, // Utiliser le nom de l'utilisateur connecté
            subsystem_id: currentUser.subsystem_id, // Utiliser le subsystem_id de l'utilisateur
            date: ticketData.date || new Date().toISOString()
        };
        
        console.log('Ticket à envoyer:', ticketToSend);
        ...
        if (response && response.success) {
            ...
            throw new Error('Échec de la sauvegarde du ticket');
        }
    } catch (error) {
...
// ==========================================
// 7. NOUVEAU: VÉRIFIER LE BLOCAGE AVANT D'OUVRIR L'ÉCRAN DE PARI
// ==========================================
...
// ==========================================
// 8. CHARGER LES DONNÉES DEPUIS L'API
// ==========================================
async function loadDataFromAPI() {
    try {
        ...
        console.log('Chargement des tickets gagnants...');
        ...
        if (winningData && winningData.success) {
            ...
            console.log(`${winningTickets.length} tickets gagnants chargés`);
        }
        
        // Charger les fiches multi-tirages
        console.log('Chargement des fiches multi-tirages...');
        ...
        if (companyData && companyData.success) {
            companyInfo = {
                name: companyData.company_name || companyInfo.name,
                phone: companyData.company_phone || companyInfo.phone,
                address: companyData.company_address || companyInfo.address,
                reportTitle: companyData.report_title || companyInfo.reportTitle,
                reportPhone: companyData.report_phone || companyInfo.reportPhone
            };
            console.log('Informations entreprise chargées:', companyInfo);
        }
        ...
        if (logoData && logoData.success && logoData.logoUrl) {
            ...
            console.log('Logo chargé:', companyLogo);
            updateLogoDisplay();
        }
        
        console.log('Données chargées depuis l\'API avec succès!');
        console.log({
        ...
        // Ne pas afficher de notification pour les erreurs réseau silencieuses
        if (!error.message.includes('Session expirée') && !error.message.includes('Network')) {
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        const savePrintMultiBtn = document.getElementById('save-print-multi-ticket');
        if (savePrintMultiBtn) {
            savePrintMultiBtn.addEventListener('click', function() {
        ...
        const viewMultiBtn = document.getElementById('view-current-multi-ticket');
        if (viewMultiBtn) {
            viewMultiBtn.addEventListener('click', function() {
        ...
        const openMultiBtn = document.getElementById('open-multi-tickets');
        if (openMultiBtn) {
            openMultiBtn.addEventListener('click', function() {
        ...
        const backMultiBtn = document.getElementById('back-from-multi-tickets');
        if (backMultiBtn) {
            backMultiBtn.addEventListener('click', function() {
        ...
        const backReportBtn = document.getElementById('back-from-report');
        if (backReportBtn) {
            backReportBtn.addEventListener('click', function() {
        ...
        const backResultsBtn = document.getElementById('back-from-results');
        if (backResultsBtn) {
            backResultsBtn.addEventListener('click', function() {
        ...
        const openResultsBtn = document.getElementById('open-results-check');
        if (openResultsBtn) {
        ...
        if (checkWinnersBtn) {
            checkWinnersBtn.addEventListener('click', function() {
        ...
        // Multi-tirages
        const multiDrawToggle = document.getElementById('multi-draw-toggle');
        if (multiDrawToggle) {
            multiDrawToggle.addEventListener('click', function() {
        ...
        const addMultiDrawBtn = document.getElementById('add-to-multi-draw');
        if (addMultiDrawBtn) {
            addMultiDrawBtn.addEventListener('click', function() {
        ...
        if (showAllTicketsBtn) {
            showAllTicketsBtn.addEventListener('click', function() {
                console.log("Afficher toutes les fiches");
                showAllTickets();
            });
        }
        
        const showPendingBtn = document.getElementById('show-pending-tickets');
        if (showPendingBtn) {
        ...
        if (searchWinningBtn) {
            searchWinningBtn.addEventListener('click', function() {
...
// ==========================================
// 10. FONCTIONS AUXILIAIRES (restent inchangées mais incluses pour complétude)
// ==========================================

// Fonction pour sauvegarder une fiche multi-tirages via API
async function saveMultiDrawTicketAPI(ticket) {
    try {
        console.log('Sauvegarde fiche multi-tirages:', ticket);
...
// Fonction pour sauvegarder l'historique via API
...
// Fonction pour charger les fiches multi-tirages depuis l'API
...
// Fonction pour sauvegarder les fiches multi-tirages via API
...
// Fonction pour ajouter un pari à la fiche multi-tirages
...
function openMultiTicketsScreen() {
    ...
    const multiTicketsScreen = document.getElementById('multi-tickets-screen');
    if (multiTicketsScreen) {
        multiTicketsScreen.style.display = 'block';
    }
    ...
    switch(betType) {
        ...
        case 'lotto4':
            ...
            setTimeout(() => {
                ...
                if (betTypeNav) betTypeNav.style.display = 'none';
                if (autoButtons) autoButtons.style.display = 'none';
                if (gamesInterface) gamesInterface.style.display = 'block';
            }, 500);
...
function openResultsCheckScreen() {
    ...
    document.querySelector('.container').style.display = 'none';
    const resultsCheckScreen = document.getElementById('results-check-screen');
    if (resultsCheckScreen) {
        resultsCheckScreen.style.display = 'block';
    }
    ...
    const winningTicketsContainer = document.getElementById('winning-tickets-container');
    const winningSummary = document.getElementById('winning-summary');
    
    if (winningTicketsContainer) winningTicketsContainer.innerHTML = '';
    if (winningSummary) winningSummary.innerHTML = '';
}
...
function initMultiDrawPanel() {
    ...
    if (!multiDrawOptions || !multiGameSelect) return;
...
function openBettingScreen(drawId, time = null) {
    ...
    if (bettingTitle) bettingTitle.textContent = title;
    ...
    if (autoButtons) autoButtons.style.display = 'none';
    if (betForm) betForm.style.display = 'none';
    if (activeBetsEl) activeBetsEl.style.display = 'block';
...
function showAutoGameForm(gameType) {
    ...
    if (gamesInterface) gamesInterface.style.display = 'none';
    if (betTypeNav) betTypeNav.style.display = 'none';
    if (autoButtons) autoButtons.style.display = 'none';
    ...
    const useBasketBtn = document.getElementById('use-basket-balls');
    const enterManualBtn = document.getElementById('enter-manual-balls');
    const processManualBtn = document.getElementById('process-manual-balls');
    const clearBallsBtn = document.getElementById('clear-balls-btn');
    
    if (useBasketBtn) useBasketBtn.addEventListener('click', loadBasketBalls);
    if (enterManualBtn) enterManualBtn.addEventListener('click', function() {
        const manualInput = document.getElementById('manual-balls-input');
        if (manualInput) manualInput.style.display = 'block';
    });
    if (processManualBtn) processManualBtn.addEventListener('click', processManualBalls);
    if (clearBallsBtn) clearBallsBtn.addEventListener('click', function() {
        selectedBalls = [];
    ...
    if (gameType === 'auto-marriage') {
        const addAutoMarriagesBtn = document.getElementById('add-auto-marriages');
        if (addAutoMarriagesBtn) {
            addAutoMarriagesBtn.addEventListener('click', addAutoMarriages);
        }
    } else if (gameType === 'auto-lotto4') {
        const addAutoLotto4Btn = document.getElementById('add-auto-lotto4');
        if (addAutoLotto4Btn) {
            addAutoLotto4Btn.addEventListener('click', addAutoLotto4);
        }
    }
...
function processManualBalls() {
    const manualInput = document.getElementById('manual-balls');
    if (!manualInput) return;
    
    const manualInputValue = manualInput.value.trim();
    ...
    // Séparer par espaces
    const balls = manualInputValue.split(/\s+/);
    ...
    const manualBallsInput = document.getElementById('manual-balls-input');
    if (manualBallsInput) manualBallsInput.style.display = 'none';
    manualInput.value = '';
}
...
function addAutoMarriages() {
    ...
    const amountInput = document.getElementById('auto-game-amount');
...
function addAutoLotto4() {
    ...
    const amountInput = document.getElementById('auto-game-amount');
    const includeReverseCheckbox = document.getElementById('include-reverse');
    
    if (!amountInput) return;
    
    const amount = parseInt(amountInput.value);
    const includeReverse = includeReverseCheckbox ? includeReverseCheckbox.checked : false;
...
function loadBetTypesForEdit() {
    ...
    if (gamesInterface) gamesInterface.style.display = 'none';
    if (betTypeNav) betTypeNav.style.display = 'flex';
    if (autoButtons) autoButtons.style.display = 'flex';
    
    if (!betTypeNav) return;
    ...
    if (gameType === 'boulpe') {
        document.querySelectorAll('.n-ball[data-number]').forEach(ball => {
            ball.addEventListener('click', function() {
                ...
                const boulpeNumberInput = document.getElementById('boulpe-number');
                const boulpeAmountInput = document.getElementById('boulpe-amount');
                
                if (boulpeNumberInput) boulpeNumberInput.value = number;
                if (boulpeAmountInput) boulpeAmountInput.focus();
            });
        });
        
        const boAllBtn = document.getElementById('bo-all');
        if (boAllBtn) {
            boAllBtn.addEventListener('click', function() {
...
function setupGrapSelection() {
    ...
    const deselectAllBtn = document.getElementById('deselect-all-graps');
    if (deselectAllBtn) {
    ...
    const addSelectedBtn = document.getElementById('add-selected-graps');
    if (addSelectedBtn) {
...
function generateAutoMarriages() {
    ...
    const borletteBalls = activeBets.filter(bet => 
        ...
        !bet.isGroup && 
        bet.number && bet.number.length === 2
    ).map(bet => bet.number);
...
function reverseLotto4Combinations() {
    ...
    lotto4Bets.forEach(bet => {
        ...
        if (bet.number && bet.number.length === 4) {
...
async function checkConnectionBeforeSavePrint() {
    ...
    if (internetStatus && internetText) {
        if (navigator.onLine) {
            ...
            if (connectionMessage) connectionMessage.textContent = 'Pa gen koneksyon entènèt. Fiche a pa kapab enprime.';
            return;
        }
    }
...
async function checkConnectionBeforePrint() {
    ...
    const connectionMessage = document.getElementById('connection-message');
    if (connectionMessage) connectionMessage.textContent = 'Koneksyon entènèt ok. Wap kontinye...';
...
function retryConnectionCheck() {
    console.log("Réessayer connexion");
    const savePrintBtn = document.getElementById('save-print-ticket');
...
function printTicket() {
    ...
    printContent.innerHTML = `
        <div style="text-align: center; padding: 20px; border: 2px solid #000; font-family: Arial, sans-serif;">
            ...
            <p><strong>Tiraj:</strong> ${draws[lastTicket.draw].name} (${lastTicket.draw_time === 'morning' ? 'Maten' : 'Swè'})</p>
...
function searchHistory() {
    ...
    const searchTerm = document.getElementById('search-history');
...
function searchWinningTickets() {
    ...
    const searchTerm = document.getElementById('search-winning-tickets');
    if (!searchTerm) return;
...
function updateHistoryScreen() {
    ...
    if (!reportsContainer || !historyList) return;
    ...
    sortedTickets.forEach(ticket => {
        ...
        historyItem.innerHTML = `
            <div class="history-header">
                <span class="history-draw">${draws[ticket.draw].name} (${ticket.draw_time === 'morning' ? 'Maten' : 'Swè'})</span>
...
window.loadTicketForEdit = function(ticketId) {
    ...
    currentDrawTime = ticket.draw_time; // Note: server.js renvoie "draw_time" (avec underscore)
...
function searchTicket() {
    const searchTerm = document.getElementById('search-ticket-number');
    if (!searchTerm) return;
...
function showAllTickets() {
    const searchInput = document.getElementById('search-ticket-number');
...
function generateEndOfDrawReport() {
    ...
    if (!reportScreen || !reportContent) return;
